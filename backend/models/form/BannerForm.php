...
class BannerForm extends \Common\models\Options
{
    ...
    public function attributeLabels()
    {
        return [
            'name' => yii::t('cms', 'Sign'),
            'tips' => yii::t('cms', 'Description'),
            'img' => yii::t('cms', 'Image'),
            'target' => yii::t('cms', 'Target'),
            'link' => yii::t('cms', 'Jump Link'),
            'sort' => yii::t('cms', 'Sort'),
            'status' => yii::t('cms', 'Status'),
            'desc' => yii::t('cms', 'Description'),
        ];
    }
    ...
    public function beforeDelete()
    {
        if($this->getScenario() == 'type'){
            if( $this->value != '[]' && $this->value != '' ){
                $this->addError('id', yii::t('cms', 'This type exits banner'));
                return false;
            }
        }
        return true;
    ...
    public function saveBanner($id, $sign='')
    {
        ...
        if ($upload !== null) {
            ...
            if (! $upload->saveAs($fullName)) {
                $this->addError('img', yii::t('cms', 'Upload {attribute} error: ' . $upload->error, ['attribute' => yii::t('cms', 'Thumb')]) . ': ' . $fullName);
                return false;
            }
