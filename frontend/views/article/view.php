...
use yii\helpers\Url;
use cms\frontend\assets\ViewAsset;
...
$categoryName = $model->category ? $model->category->name : yii::t('cms', 'uncategoried');
...
<div class="content-wrap">
    <div class="content">
        <div class="breadcrumbs">
            <a title="<?=yii::t('cms', 'Return Home')?>" href="<?= yii::$app->getHomeUrl() ?>"><i class="fa fa-home"></i></a>
            <small>&gt;</small>
        ...
        <article class="article-content">
            ...
            <p>
                <?= yii::t('cms', 'Reproduced please indicate the source') ?>：
            ...
            <div class="article-social">
                <a href="javascript:;" data-action="ding" data-id="<?=$model->id?>" like-url="<?=Url::to(['article/like'])?>" id="Addlike" class="action"><i class="fa fa-heart-o"></i><?=yii::t('cms', 'Like')?> (<span class="count"><?= $model->getArticleLikeCount() ?></span>)</a>
                <span class="or">or</span>
                <span class="action action-share bdsharebuttonbox"><i class="fa fa-share-alt"></i><?=yii::t('cms', 'Share')?> (<span class="bds_count" data-cmd="count" title="累计分享0次">0</span>)
        ...
        <div class="related_top">
            <div class="related_posts">
                <ul class="related_img">
                    <h2><?= yii::t('cms', 'Related Recommends') ?></h2>
                    <?php
            ...
            <form action="" method="post" id="commentform">
                ...
                <div class="comt-title" style="display: block;">
                    ...
                    <div class="comt-author pull-left"><?php if (yii::$app->getUser()->getIsGuest()) {
                            echo yii::t('cms', 'Guest');
                        } else {
                            echo yii::$app->getUser()->getIdentity()->username;
                        } ?> <span><?= yii::t('cms', 'Post my comment') ?></span> &nbsp;
                        <a class="switch-author" href="javascript:void(0)" data-type="switch-author" style="font-size:12px;"><?= yii::t('cms', 'Change account') ?></a>
                    </div>
                    <a id="cancel-comment-reply-link" class="pull-right" href="javascript:;"><?= yii::t('cms', 'Cancel comment') ?></a>
                </div>
                ...
                <div class="comt">
                    <div class="comt-box">
                        <?= $form->field($commentModel, 'content', ['template' => '{input}'])->textarea([
                            ...
                            'rows' => '3',
                            'tabindex' => 1,
                            'placeholder' => yii::t('cms', 'Writing some...'),
                            "id" => "comment"
                        ]) ?>
                        <div class="comt-ctrl">
                            <button class="btn btn-primary pull-right" type="submit" name="submit" id="submit" tabindex="5">
                                <i class="fa fa-check-square-o"></i> <?= yii::t('cms', 'Submit comment') ?>
                            </button>
                            ...
                            <span data-type="comment-insert-smilie" class="muted comt-smilie"><i class="fa fa-smile-o"></i> <?= yii::t('cms', 'emoj') ?></span>
                            <span class="muted comt-mailme"><label for="comment_mail_notify" class="checkbox inline" style="padding-top:0">
                                <input type="checkbox" name="comment_mail_notify" id="comment_mail_notify" value="comment_mail_notify" checked="checked"><?=yii::t('cms', 'Send email at someone replied')?></label>
                            </span>
                        </div>
                    </div>
                    <div class="comt-comterinfo" id="comment-author-info" style="display:none">
                        <h4><?= yii::t('cms', 'Hi, Please fill') ?></h4>
                        <ul>
                            <li class="form-inline">
                                <label class="hide" for="author"><?= yii::t('cms', 'Nickname') ?></label>
                                <?php if (yii::$app->user->isGuest) {
                                    $defaultNickname = yii::t('cms', 'Guest');
                                } else {
                                    $defaultNickname = yii::$app->getUser()->getIdentity()->username;
                                } ?>
                                <?= $form->field($commentModel, 'nickname', ['template' => '{input}<span class="help-inline">' . yii::t('cms', 'Nickname') . ' (' . yii::t('cms', 'required') . ')</span>'])->textInput(['value' => $defaultNickname]) ?>
                            </li>
                            <li class="form-inline"><?= $form->field($commentModel, 'email', ['template' => '{input}<span class="help-inline">' . yii::t('cms', 'Email') . ' </span>'])->textInput() ?></li>
                            <li class="form-inline"><?= $form->field($commentModel, 'website_url', ['template' => '{input}<span class="help-inline">' . yii::t('cms', 'Website') . '</span>'])->textInput() ?></li>
                        </ul>
                    </div>
                </div>
                <?php ActiveForm::end() ?>
        ...
        <div id="postcomments">
            <div id="comments">
                <i class="fa fa-comments-o"></i> <b> (<?= $model->comment_count ?>)</b><?= yii::t('cms', 'person posted') ?>
            </div>
                                    ...
                                    <li class="comment odd alt depth-2" id="comment-<?= $value['id'] ?>">
                                        <div class="c-avatar">
                                            ...
                                            <div class="c-main" id="div-comment-<?= $value['id'] ?>"><?= $value['content'] ?><br>
                                                <div class="c-meta">
                                                    <span class="c-author">
                                                        <a href="<?= $v['website_url'] ?>" rel="external nofollow" class="url" target="_blank"><?= empty($value['nickname']) ? yii::t('cms', "Guest") : $value['nickname'] ?></a>
                                                    </span>
